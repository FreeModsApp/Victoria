...
from VictoriaBot.conf import get_int_key, get_str_key

# from VictoriaBot.db.mongo_helpers.nsfw_guard import add_chat, get_all_nsfw_chats, is_chat_in_db, rm_chat
from VictoriaBot.pyrogramee.telethonbasics import is_admin
from VictoriaBot.events import register
from VictoriaBot import MONGO_DB_URI 
from pymongo import MongoClient
from VictoriaBot.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
    get_all_nsfw_enabled_chat,
    is_nsfwatch_indb,
    rmnsfwatch,
)
from VictoriaBot import telethn as tbot
...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Victoria deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By Victoria AI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, Victoria deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By VictoriaAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
        ...
        if event.photo:
            if event.chat_id == c["id"]:
                ...
                if nude.is_nude("./nudes.jpg"):
                    ...
                    st = sender.first_name
                    hh = sender.id
                    final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Victoria deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By Victoria` \n**#GROUP_GUARDIAN** "
                    dev = await event.respond(final)
                    await asyncio.sleep(10)
                    await dev.delete()
                    os.remove("nudes.jpg")
...
__help__ = """
<b> Group Guardian: </b>
✪ Victoria can protect your group from NSFW senders, Slag word users and also can force members to use English
